public class PongComplete {
    public static void main(String[] args) {
        ...
        double ball_xvel = 0.032 * (Math.random() < 0.5 ? -1 : 1);    // Random ball x velocity on start
        double ball_yvel = 0.018 * (Math.random() < 0.5 ? -1 : 1);    // Random ball y velocity on start
        ...
        while (winner < 0)
        {
            ...
            // Top and bottom
            if (ball_y + BALL_RADIUS >= UPPER_PDL_yLIM ||
            ...
            else if (ball_x + BALL_RADIUS >= UPPER_PDL_xLIM) {
                winner = 1;
            }
            ...
            if ( (ball_y > p1_pdl_y) &&
                 (ball_y < p1_pdl_y + PADDLE_HEIGHT) &&
                 (ball_x - BALL_RADIUS < PADDLE_1_X + PADDLE_WIDTH) &&
                 //(ball_x > PADDLE_1_X + PADDLE_WIDTH) &&

                 (ball_xvel < 0) ) {
                ball_xvel = -ball_xvel;
            }
            else if (ball_y > p2_pdl_y &&
                    ball_y < p2_pdl_y + PADDLE_HEIGHT &&
                    ball_x + BALL_RADIUS > PADDLE_2_X &&
                    //ball_x < PADDLE_2_X  &&
                    ball_xvel > 0) {
                ball_xvel = -ball_xvel;
            }
